...
const CreateAdmin: React.FC = () => {
  ...
  return (
    <div className="container mx-auto px-4 py-10 max-w-xl">
      ...
      <Card>
        <CardHeader><CardTitle>Create a new admin</CardTitle></CardHeader>
        <CardContent>
      ...
      <Dialog open={!!result} onOpenChange={(o) => { if (!o) { setResult(null); navigate('/super-admin/dashboard'); } }}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2"><KeyRound className="w-5 h-5 text-primary" /> Admin created</DialogTitle>
          </DialogHeader>
          <p className="text-sm text-muted-foreground">
            Share these credentials with the new admin now. The password is shown only once.
          </p>
          <div className="rounded-lg border bg-muted/40 p-4 space-y-2 text-sm">
            <div className="flex justify-between"><span className="text-muted-foreground">Username</span><span className="font-medium">{result?.name}</span></div>
            <div className="flex justify-between"><span className="text-muted-foreground">Tracking prefix</span><span className="font-mono">{result?.prefix}</span></div>
            <div className="flex justify-between"><span className="text-muted-foreground">Temp password</span><span className="font-mono font-semibold">{result?.tempPassword}</span></div>
          </div>
          <DialogFooter>
            <Button variant="outline" onClick={copy}><Copy className="w-4 h-4 mr-2" /> Copy</Button>
            <Button onClick={() => { setResult(null); navigate('/super-admin/dashboard'); }}>Done</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
