    ...
    <Dialog open={open} onOpenChange={(v) => !v && onClose()}>
      <DialogContent className="max-w-3xl max-h-[92vh] overflow-y-auto p-0 bg-background">
        ...
        <div className="sticky top-0 z-10 flex items-center justify-between px-4 py-2 border-b bg-background">
          <div className="text-sm font-semibold text-foreground">Ticket Preview</div>
          ...
          <div className="mx-auto bg-white text-slate-900 shadow-lg" style={{ width: '100%', maxWidth: 720 }}>
            ...
            <div style={{ background: '#1e3a5f', color: '#fff', padding: '20px 24px', position: 'relative' }}>
              ...
              <div style={{ position: 'absolute', top: 20, right: 24, background: accent, padding: '8px 14px', borderRadius: 4, fontWeight: 700, fontSize: 12 }}>
                {isPaid ? 'PAYMENT RECEIPT' : 'INVOICE — DUE'}
              </div>
            ...
            <div style={{ padding: '20px 24px', display: 'flex', justifyContent: 'space-between' }}>
              <div>
                <div style={{ fontSize: 20, fontWeight: 700, color: '#1e3a5f' }}>
                  {ticket.title || (isPaid ? 'Payment Receipt' : 'Pending Payment')}
                </div>
                <div style={{ fontSize: 12, color: '#666', marginTop: 6 }}>Ticket #: <strong>{ticket.ticketNumber}</strong></div>
                <div style={{ fontSize: 12, color: '#666' }}>Issued: {new Date(ticket.createdAt).toLocaleDateString()}</div>
                {ticket.dueDate && <div style={{ fontSize: 12, color: '#b45309', fontWeight: 600 }}>Due: {new Date(ticket.dueDate).toLocaleDateString()}</div>}
              </div>
              {qr && (
                <div style={{ textAlign: 'center' }}>
                  <img src={qr} alt="QR" style={{ width: 90, height: 90 }} />
                  <div style={{ fontSize: 9, color: '#888', marginTop: 2 }}>Scan to track</div>
                </div>
              )}
            </div>
            ...
            <div style={{ padding: '0 24px' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 24, padding: '14px 0' }}>
                <div>
                  <div style={{ fontSize: 10, fontWeight: 700, color: '#1e3a5f', letterSpacing: 1 }}>BILL TO</div>
              ...
              {shipmentInfo && (
                <div style={{ background: '#f8fafc', border: '1px solid #e5e7eb', padding: 12, borderRadius: 6, marginBottom: 14 }}>
                  <div style={{ fontSize: 10, fontWeight: 700, color: '#1e3a5f', letterSpacing: 1, marginBottom: 4 }}>SHIPMENT DETAILS</div>
                  <div style={{ fontSize: 12 }}>Tracking #: <strong>{shipmentInfo.trackingNumber}</strong></div>
                  <div style={{ fontSize: 12 }}>Route: {shipmentInfo.origin} → {shipmentInfo.destination}</div>
                </div>
              )}
            </div>
            ...
            <div style={{ padding: '0 24px' }}>
              <table style={{ width: '100%', borderCollapse: 'collapse', marginBottom: 16 }}>
                <thead>
                  <tr style={{ background: '#1e3a5f', color: '#fff' }}>
                    <th style={{ textAlign: 'left', padding: '8px 12px', fontSize: 11 }}>DESCRIPTION</th>
                    <th style={{ textAlign: 'right', padding: '8px 12px', fontSize: 11 }}>AMOUNT</th>
                  </tr>
                </thead>
                <tbody>
              ...
              <div style={{ display: 'flex', justifyContent: 'flex-end' }}>
                <table style={{ minWidth: 260 }}>
                  <tbody>
                    <tr><td style={{ fontSize: 12, color: '#555', padding: '3px 8px' }}>Subtotal</td><td style={{ fontSize: 12, textAlign: 'right', padding: '3px 0' }}>{ticket.currency} {subtotal.toFixed(2)}</td></tr>
                    {discount > 0 && <tr><td style={{ fontSize: 12, color: '#555', padding: '3px 8px' }}>Discount</td><td style={{ fontSize: 12, textAlign: 'right', padding: '3px 0', color: '#dc2626' }}>- {ticket.currency} {discount.toFixed(2)}</td></tr>}
                    {taxRate > 0 && <tr><td style={{ fontSize: 12, color: '#555', padding: '3px 8px' }}>Tax ({taxRate}%)</td><td style={{ fontSize: 12, textAlign: 'right', padding: '3px 0' }}>{ticket.currency} {tax.toFixed(2)}</td></tr>}
                    <tr style={{ borderTop: '2px solid #1e3a5f' }}>
                      <td style={{ fontSize: 14, fontWeight: 700, color: '#1e3a5f', padding: '6px 8px' }}>TOTAL</td>
              ...
              {ticket.paymentMethod && (
                <div style={{ marginTop: 16 }}>
                  <div style={{ fontSize: 10, fontWeight: 700, color: '#1e3a5f', letterSpacing: 1 }}>{isPaid ? 'PAYMENT METHOD' : 'PAYMENT INSTRUCTIONS'}</div>
              ...
              {ticket.notes && (
                <div style={{ marginTop: 12 }}>
                  <div style={{ fontSize: 10, fontWeight: 700, color: '#1e3a5f', letterSpacing: 1 }}>NOTES</div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 32, marginTop: 8, paddingBottom: 16 }}>
                <div>
                  <div style={{ borderTop: '1px solid #ccc', paddingTop: 4, fontSize: 10, color: '#888' }}>Authorized Signature</div>
                </div>
                <div>
                  <div style={{ borderTop: '1px solid #ccc', paddingTop: 4, fontSize: 10, color: '#888' }}>Client Acknowledgement</div>
                </div>
              </div>
            </div>
